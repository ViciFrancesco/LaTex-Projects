...
\newenvironment{oldcode}[1]					%TODO Da cancellare
{
\textbf{#1}
%\algorithmicindent{2em}
\begin{algorithmic}[1]
}{
\end{algorithmic}
}
...
%TODO Finire sostituzione pseudocodice 
		
		
		
		\begin{oldcode}{Merge(A,p,q,r)}
\STATE $n_1 \gets q-p+1$
\STATE $n_2 \gets r-q$
\FOR{$i \gets 1 \text{ to } n_1$} 
	\STATE $L[i] \gets A[p+i-1]$		$\qquad$\COMMENT {Crea il sottoarray sinistro}
\ENDFOR
\FOR{$j \gets 1 \text{ to } n_2$} 
	\STATE $R[j] \gets A[q+j]$			$\qquad$\COMMENT{Crea il sottoarray destro}
\ENDFOR
...
Divide ricorsivamente l'array da ordinare in due sottoarray grazie all'uso di due indici (ordina sul posto) grazie all'algoritmo "partition" per poi riordinarli e combinarli grazie all'algoritmo "quicksort". Lo pseudocodice è:
		\begin{oldcode}{QuickSort(A,p,r)}
\IF{$p<r$}
	\STATE $q \gets \text{\bo{Partition}(A,p,r)}$
	\STATE \bo{QuickSort}(A,p,q-1)
	\STATE \bo{QuickSort}(A,q+1,r)
\ENDIF
		\end{oldcode}
		\begin{oldcode}{Partition(A,p,r)}
\STATE $x\gets A[r]$
\STATE $i\gets p-1$
\FOR{$j\gets p \text{ to }r-1$}
	\IF{$A[j]\leq x$}
		\STATE $i \gets i+1$
		\STATE \bo{exchange }$A[i] \leftrightarrow A[j]$
	\ENDIF
\ENDFOR
\STATE \bo{exchange }$A[i+1]\leftrightarrow A[r]$
\STATE \bo{return }$i+1$
		\end{oldcode}
\bo{Costo}\\
		\begin{descr}{1}
...
Poiché non posso conoscere l'ordine in cui l'algoritmo riceverà gli input, non posso, allo stesso modo, conoscere la probabilità delle $n!$ possibili permutazioni. Uso l'algoritmo "random" per forzare l'ordine casuale. Lo pseudocodice è: 
		\begin{oldcode}{RandomizeInPlace(A)}
\STATE $n \gets A.lenght$
\FOR{$i \gets 1 \text{ to }n$}
	\STATE $\text{\bf{excange }} A[i]\leftrightarrow A[\text{\bf{Random}}(i,n)]$
\ENDFOR 
		\end{oldcode}
\bo{Costo:}\\
...
Cerco di avvicinarmi il più possibile all'ipotesi di caso medio, per farlo randomizzo la scelta del pivot $q$ per l'algoritmo "partition". Lo pseudocodice è: 
		...
		\begin{oldcode}{RandPartition(A,p,r)}
\STATE $i\gets$ \bo{Random}(p,r)
\STATE \bo{exchange} $A[r]\leftrightarrow A[i]$
\STATE \bo{return Partition} (A,p,r)
		\end{oldcode}
		
		
		\begin{oldcode}{RandQuickSort(A,p,r)}
\IF{$p<r$}
	\STATE $q\gets$ \bo{RandomizedPartition}(A,p,r)
	\STATE	 \bo{RandomizedQuickSort}(A,p,q-1)
	\STATE \bo{RandomizedQuickSort}(A,q+1,r)
\ENDIF
		\end{oldcode}
\bo{Costo}\\
...
È un algoritmo di ordinamento che non si basa sui confronti, ma richiede due array di supporto per effettuare l'ordinamento. Inoltre può ordinare solo numeri naturali. Lo pseudocodice è: \\
		\begin{oldcode}{CountingSort(A,B,k)}
\FOR{$i\gets 0\text{ to }k$}
	\STATE $C[i]\gets0$
\ENDFOR
\FOR{$j\gets 1\text{ to }A.lenght$}
	\STATE $C[A[j]]\gets C[A[j]]+1$
\ENDFOR
\FOR{$i\gets 1\text{ to }k$}
	\STATE $C[i]\gets C[i]+C[i-1]$
\ENDFOR
\FOR{$j\gets A.lenght\text{ downto }1$}
...
È un algoritmo di ordinamento che usa il concetto controintuitivo di ordinare le singole cifre a partire dalla meno significativa. Necessita inoltre di riceve in ingresso in numero di cifre "d". Lo pseudocodice è: \\
		\begin{oldcode}{RadixSort(A,d)}
\FOR{$i \gets 1 \text{ to }d$}
	\STATE "usa ordinamento stabile per ordinare l'array $A$ sulla cifra $i$"
\ENDFOR
		\end{oldcode}
