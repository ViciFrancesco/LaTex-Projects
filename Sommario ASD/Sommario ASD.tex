...
È uno degli algoritmi iterativi più semplici. Ordina "in place" ed è efficiente per insiemi quasi ordinati. Lo pseudocodice è:
	\begin{code}{InsertionSort(A)}
\FOR{$j\gets2 \text{ to } A.lenght$}
	\STATE $key \gets A[j]$
	\STATE $i\gets j-1$
...
\FOR{$j\gets 1 \text{ to } n-1$}
	...
	\FOR{$i\gets j+1\text{ to } n$}
		\IF{$A[I]<A[smallest]$}
			\STATE $smallest \gets i$
		\ENDIF
	\ENDFOR
	\STATE \textbf{exchange} $A[j]\leftrightarrow A[smallest]$
\ENDFOR
		\end{code}
\textbf{Correttezza con I.C.}
		\begin{descr}{1}
...
Cerco di avvicinarmi il più possibile all'ipotesi di caso medio, per farlo randomizzo la scelta del pivot $q$ per l'algoritmo "partition". Lo pseudocodice è: 
		...
		\begin{code}{RandPartition(A,p,r)}
...
\STATE \bo{return Partition} (A,p,r)
		...
		\begin{code}{RandQuickSort(A,p,r)}
\IF{$p<r$}
