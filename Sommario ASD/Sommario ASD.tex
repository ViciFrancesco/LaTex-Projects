...
\li \FOR{i \get 1 \TO m}
	\li \FOR{j \get 1 \TO n}
		...
		\li \IF{x_i=y_j}
			...
			\li $op[i,j] \get$ "COPY"
		\END
		\li \IF{x_i \neq y_j \text{ AND } c[i-1,j-1]+\text{cost}(\proc{replace})<c[i,j]}
		...
		\END
		\li \IF{c[i-1,j]+\text{cost}(\proc{delete})<c[i,j]}
			\li $c[i,j] \get c[i-1,j]+\text{cost}(\proc{delte})$
			\li $op[i,j] \get$ "DELETE"
		\END
		\li \IF{c[i,j-1]+\text{cost}(\proc{insert})<c[i,j]}
...
Sia $S\subset V$ un qualunque insieme di vertici tale che $c\in S$ e $f\in V-S$. Ogni MST deve avere almeno un arco che connetta $S$ con $V-S$, voglio scegliere l'arco che li connette con il peso minore.
				\item[Definizioni]:\\
				\begin{Descr} 
					\item[taglio] $\ra$ Dato un grafo \G non orientato e definisco $S\subset V$ e $A\subseteq E$. Allora un taglio $(S,V-S)$ è una partizione dei vertici $V$ in due insiemi disgiunti $S$ e $V-S$. Un arco $(u,v)$ \bo{attraversa} un taglio \SSE $u\in S$ e $v\in V-S$ (o viceversa). Infine un arco \bo{rispetta} un insieme di archi $A$ \SSE nessun arco attraversa il taglio. 
